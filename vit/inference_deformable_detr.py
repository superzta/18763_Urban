"""
Deformable DETR Inference Example Script
========================================
Simple script to run inference on your own images or test images.

Usage:
    python inference_detr_example.py
"""
...
# Reuse Config and dataset metadata from rcnn
from rcnn import Config
# Import Deformable DETR inference function
# NOTE: if your file is named differently (e.g. deformable_detr.py),
# change `detr` below to the correct module name.
from deformable_detr_pretrained import inference as detr_inference


def inference_on_custom_image(config, checkpoint_path):
    """Run inference on user-provided image using Deformable DETR."""
    print("\n" + "=" * 80)
    print("Custom Image Inference (Deformable DETR)")
    print("=" * 80)
    ...
    save_path = os.path.join(
        config.output_dir,
        f'{image_name}_detr_inference_{timestamp}.jpg'
    )
    ...
    boxes, labels, scores = detr_inference(
        config,
        checkpoint_path,
        image_path,
        save_path
...
def inference_on_test_images(config, checkpoint_path):
    """
    Run inference on random test images from the dataset using Deformable DETR.
    Uses the same folder layout as RCNN (config.URBAN_ISSUE_DATASETS).
    """
    print("\n" + "=" * 80)
    print("Test Images Inference (Deformable DETR)")
    print("=" * 80)
    ...
    for i, (img_path, class_id, class_name) in enumerate(all_test_images, 1):
        ...
        save_path = os.path.join(
            config.output_dir,
            f'detr_test_class{class_id}_{i}_{timestamp}.jpg'
        )

        # Run inference
        print(f"\n[{i}/{len(all_test_images)}] {img_path.name}")
        print(f"  Ground truth class (dataset folder): {class_name}")
    ...
    # Summary
    print("\n" + "=" * 80)
    print(f"✓ Inference completed (Deformable DETR)!")
...
def main():
    """Main Deformable DETR inference script."""
    print("=" * 80)
    print("RUIDR - Deformable DETR Inference Tool")
    print("=" * 80)
    ...
    user_input = input("\nEnter class IDs your DETR model was trained on (e.g., '3' or '0,1,3'): ").strip()
    ...
    # Checkpoint path (default to your DETR best checkpoint)
    default_ckpt = "checkpoints/deformable_detr_best.pth"
    checkpoint_input = input(f"Checkpoint path (default: {default_ckpt}): ").strip()
    checkpoint_path = checkpoint_input if checkpoint_input else default_ckpt

    if not os.path.exists(checkpoint_path):
        print(f"\nError: Checkpoint not found at {checkpoint_path}")
        print("Please train the Deformable DETR model first or specify a valid checkpoint path.")
        return
